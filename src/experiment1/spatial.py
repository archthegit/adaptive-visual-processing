...
from .token_layout import TokenLayout, VisualTokenCell
...
def build_spatial_relevance_from_token_scores(
    ...
    temporal_indices = sorted({int(item["temporal_index"]) for item in mappings})
    ...
    return {
        ...
        "temporal_frame_indices": temporal_indices,
        "normalized_spatial_distribution_by_temporal_frame": by_frame,
        "normalized_global_spatial_distribution": normalized_global.tolist(),
        "metadata": {
            "num_layers": int(raw.shape[0]),
            "num_visual_tokens": int(raw.shape[1]),
            "num_temporal_frames": int(len(temporal_indices)),
...
def spatial_route_spec_from_baseline_artifact(
    ...
    *,
    ...
    per_frame_budget = {str(frame): _retained_count(tokens) for frame, tokens in frame_groups.items()}
    ...
    for anchor in anchors:
        allowed: list[int] = []
        selected_by_frame: dict[str, list[int]] = {}
        for frame, frame_tokens in frame_groups.items():
            ...
            selected_by_frame[str(frame)] = selected
            allowed.extend(selected)
        route = _route_for_allowed(allowed, all_tokens, anchor)
        route["selected_visual_tokens_by_temporal_frame"] = selected_by_frame
        route["per_frame_retained_visual_tokens"] = dict(per_frame_budget)
    ...
    return {
        ...
        "temporal_frames_preserved": sorted(int(frame) for frame in frame_groups),
        "per_frame_retained_visual_tokens": dict(per_frame_budget),
        ...
        "mask_rule": (
            ...
            "indices at routed decoder layers. Every temporal frame retains its own 50% spatial budget."
        ),
    }


def validate_spatial_route_spec(route: Mapping[str, Any]) -> None:
    ...
    frames = set(int(item) for item in route.get("temporal_frames_preserved", ()))
    if not frames:
        raise ValueError("Spatial route must preserve at least one temporal frame.")
    budgets = {int(k): int(v) for k, v in (route.get("per_frame_retained_visual_tokens") or {}).items()}
    if set(budgets) != frames:
        raise ValueError("Spatial route per-frame budgets must match preserved temporal frames.")
    for layer, layer_route in (route.get("layer_routes") or {}).items():
        by_frame = {int(k): [int(token) for token in v] for k, v in (layer_route.get("selected_visual_tokens_by_temporal_frame") or {}).items()}
        if set(by_frame) != frames:
            raise ValueError(f"Spatial route layer {layer} dropped a temporal frame.")
        for frame, tokens in by_frame.items():
            if len(tokens) != budgets[frame]:
                raise ValueError(f"Spatial route layer {layer} has wrong budget for frame {frame}.")
